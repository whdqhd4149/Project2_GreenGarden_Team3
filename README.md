...
### 1. JPA DDL 설정으로 인한 데이터 초기화

#### 문제

애플리케이션을 재실행한 후 Oracle 데이터베이스에 저장되어 있던 회원정보와 테스트 데이터가 모두 사라지는 문제가 발생했습니다.

#### 원인

`application.yml`의 JPA 설정이 `ddl-auto: create`로 지정되어 있어 애플리케이션을 실행할 때마다 기존 테이블을 삭제하고 다시 생성하고 있었습니다.

#### 해결

기존 테이블과 데이터를 유지할 수 있도록 설정을 `update`로 변경했습니다.
...
설정 변경 후 애플리케이션을 다시 실행해도 기존 데이터가 유지되는 것을 확인했습니다.

---

### 2. CSRF 토큰 누락으로 인한 403 오류

#### 문제

Spring Security 적용 후 로그인, 일반회원·판매자 회원가입, 로그아웃 POST 요청에서 `403 Forbidden` 오류가 발생했습니다.

#### 원인

Spring Security의 CSRF 보호 기능이 활성화되어 있었지만 각 Form에 CSRF 토큰이 포함되어 있지 않아 요청이 처리되기 전에 차단되고 있었습니다.

#### 해결

로그인·회원가입 Form과 공통 헤더의 로그아웃 Form에 Thymeleaf가 제공하는 CSRF 토큰을 추가했습니다.
...
토큰 적용 후 로그인, 일반회원·판매자 회원가입, 공통 헤더의 로그아웃 기능이 403 오류 없이 정상적으로 처리되는 것을 확인했습니다.
