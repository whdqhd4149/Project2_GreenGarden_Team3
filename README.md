...
![사용자 메인페이지 상단](docs/images/Main1.jpg)

#### 주요 상품 영역

![사용자 메인페이지 상품 영역](docs/images/Main2.jpg)
...
![GreenGarden ERD 1](docs/images/ERD1.jpg)

![GreenGarden ERD 2](docs/images/ERD2.jpg)
...
![일반회원 가입 화면](docs/images/GeneralRegister.jpg)
...
![판매자 회원가입 화면](docs/images/SellerRegister.jpg)
...
### JPA DDL 자동 생성 설정으로 인한 데이터 초기화

#### 문제 상황

프로젝트 초기에는 데이터베이스에 저장된 테스트 데이터가 많지 않아 JPA의 DDL 자동 생성 기능을 사용했습니다.

개발 과정에서 애플리케이션을 재실행한 후 Oracle 데이터베이스에 저장되어 있던 회원정보와 테스트 데이터가 모두 사라지는 문제가 발생했습니다.

#### 원인 분석

`application.yml`을 확인한 결과, JPA의 DDL 자동 생성 설정이 다음과 같이 지정되어 있었습니다.
...
`ddl-auto: create`는 애플리케이션을 실행할 때 기존 테이블을 삭제한 후 Entity를 기준으로 테이블을 다시 생성합니다.

이로 인해 애플리케이션을 다시 실행할 때마다 기존 테이블이 재생성되면서 저장되어 있던 데이터도 함께 삭제된다는 것을 확인했습니다.

#### 해결 방법

기존 테이블과 데이터를 유지하면서 Entity의 변경된 구조를 반영할 수 있도록 설정값을 `create`에서 `update`로 변경했습니다.
...
설정을 변경한 후 애플리케이션을 재실행해도 기존 테이블과 데이터가 유지되는 것을 확인했습니다.

개발 중 생성한 회원정보와 테스트 데이터를 다시 입력해야 하는 문제도 방지할 수 있었습니다.

#### 배운 점

JPA의 DDL 자동 생성 설정은 단순히 테이블 생성 여부만 결정하는 것이 아니라 기존 테이블과 데이터에도 직접적인 영향을 준다는 점을 배웠습니다.

특히 `create`와 `create-drop`은 애플리케이션 실행 과정에서 기존 테이블을 삭제할 수 있기 때문에 초기 테스트 환경에서만 주의해서 사용해야 한다는 것을 알게 되었습니다.

이후에는 데이터베이스 설정을 변경할 때 각 옵션의 동작을 먼저 확인하고, 기존 데이터에 미치는 영향을 고려한 뒤 적용하는 습관을 갖게 되었습니다.

---

### Spring Security CSRF 보호로 인한 회원 기능 403 오류

#### 문제 상황

Spring Security를 적용한 후 로그인과 회원가입 화면에는 정상적으로 접근할 수 있었지만, Form을 제출하면 `403 Forbidden` 오류가 발생했습니다.

공통 헤더의 로그아웃 버튼을 눌렀을 때도 동일한 403 오류가 발생했습니다.

오류가 발생한 기능은 다음과 같았습니다.
...
요청 경로와 Spring Security 설정을 확인했지만 로그인, 회원가입, 로그아웃 주소는 정상적으로 연결되어 있었습니다.

화면을 조회하는 GET 요청은 정상적으로 처리됐지만, 데이터를 전송하거나 로그인 상태를 변경하는 POST 요청에서 공통적으로 오류가 발생했습니다.

#### 원인 분석

Spring Security의 CSRF 보호 기능이 활성화된 상태에서는 서버의 상태를 변경하는 POST 요청에 유효한 CSRF 토큰이 포함되어야 합니다.

그러나 로그인, 회원가입, 로그아웃 Form에 CSRF 토큰이 포함되어 있지 않아 요청이 실제 처리 과정에 도달하기 전에 Spring Security의 CSRF 필터에서 차단되고 있었습니다.

또한 회원 관련 경로에 `permitAll()`을 설정했더라도 이는 비로그인 사용자의 접근을 허용하는 인가 설정일 뿐, CSRF 검사를 생략하는 설정은 아니라는 점을 확인했습니다.

#### 해결 방법

로그인, 일반회원 가입, 판매자 회원가입 Form에 Thymeleaf가 제공하는 CSRF 토큰을 hidden 값으로 추가했습니다.
...
다음 파일의 Form에 CSRF 토큰을 적용했습니다.

- `login.html`
- `register.html`
- `registerSeller.html`

로그아웃 기능은 여러 화면에서 공통 헤더를 통해 사용하고 있었기 때문에 개별 페이지가 아니라 실제 로그아웃 Form이 위치한 공통 헤더 파일을 수정했습니다.
...
다음 공통 헤더 파일에 동일한 처리를 적용했습니다.

- `header_index.html`
- `headerWithSearch.html`
- `headerNoSearch.html`

#### 결과

2025년 10월 17일 각 Form에 CSRF 토큰 처리를 적용한 후 다음 기능의 403 오류가 해결되었습니다.

- 일반 로그인 정상 처리
- 일반회원·판매자 회원가입 정상 처리
- 공통 헤더를 통한 로그아웃 정상 처리

공통 헤더에서 로그아웃 기능을 수정하여 해당 헤더를 사용하는 여러 화면에 동일한 처리를 일관되게 적용할 수 있었습니다.

#### 배운 점

Spring Security의 CSRF 필터는 Controller보다 먼저 요청을 검사하기 때문에 요청 경로가 정확하더라도 CSRF 토큰이 없으면 POST 요청이 차단될 수 있다는 점을 배웠습니다.

또한 `permitAll()`은 비로그인 사용자의 접근을 허용하는 설정이며, CSRF 검사를 해제하는 설정은 아니라는 차이를 이해하게 되었습니다.

이번 문제를 통해 동일한 403 오류가 여러 기능에서 발생하면 각각의 Controller와 요청 경로만 확인하는 것이 아니라, 요청을 공통으로 검사하는 Spring Security 설정도 함께 확인해야 한다는 점을 경험했습니다.
